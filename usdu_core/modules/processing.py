...
from usdu_utils import tensor_to_pil, pil_to_tensor, get_crop_region, expand_crop, crop_cond
...
def sample(model, seed, steps, cfg, sampler_name, scheduler, positive, negative, latent, denoise, custom_sampler, custom_sigmas, suppress_preview=False):
    ...
    # Silent Sampling (No Preview / Progress)
    if suppress_preview:
        # Replicate common_ksampler logic but without callback
        ...
        samples_tensor = comfy.sample.sample(
            ...
            force_full_denoise=False, noise_mask=noise_mask, callback=None, disable_pbar=disable_pbar, seed=seed
        )
    ...
    # Default (Calls common_ksampler which adds callback for preview)
...
def process_images(p: StableDiffusionProcessing) -> Processed:
    ...
    # Generate samples
    suppress_preview = getattr(p, 'suppress_preview', False)
    samples = sample(p.model, p.seed, p.steps, p.cfg, p.sampler_name, p.scheduler, positive_cropped,
                     negative_cropped, latent, p.denoise, p.custom_sampler, p.custom_sigmas, suppress_preview=suppress_preview)
