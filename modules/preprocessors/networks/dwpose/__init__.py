...
import warnings
from ..aux_util import HWC3, resize_image_with_pad, common_input_validate
import cv2
...
class DwposeDetector:
    ...
    def from_pretrained(cls, pretrained_model_or_path, pretrained_det_model_or_path=None, det_filename=None, pose_filename=None, torchscript_device="cuda"):
        ...
        if det_filename is not None:
            det_model_path = os.path.join(pretrained_det_model_or_path, det_filename)
        pose_model_path = os.path.join(pretrained_model_or_path, pose_filename)
        
        print(f"\nDWPose: Using {det_model_path} for bbox detection and {pose_model_path} for pose estimation")
